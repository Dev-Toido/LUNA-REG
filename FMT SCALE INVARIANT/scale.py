# """
# Fourier-Mellin Transform (FMT) + Pyramid-aware SuperPoint + LightGlue registration.

# WHAT'S NEW vs pyramid_lightglue_registration.py
# -------------------------------------------------
# Previously you had to manually set SOURCE_RESOLUTION_M / REFERENCE_RESOLUTION_M
# by hand -- and if you guessed wrong (as happened before), the pyramid would
# downsample one image far too aggressively and matching would collapse to
# almost zero matches.

# This version adds a Fourier-Mellin Transform step that AUTOMATICALLY
# estimates the scale ratio (and rotation) between the two images directly
# from their content, before the pyramid is built. That estimated scale
# ratio is then used to set the pyramid's resolution parameters -- so you
# don't have to know or guess the real-world sensor resolutions.

# How Fourier-Mellin works (brief):
#   1. Take each image's Fourier magnitude spectrum (this is naturally
#      invariant to translation).
#   2. Warp that magnitude spectrum into log-polar coordinates. In this
#      representation, a SCALE change between the two images becomes a
#      simple horizontal SHIFT, and a ROTATION becomes a simple vertical
#      SHIFT.
#   3. Run phase correlation (same building block as before) on the two
#      log-polar spectra to find that shift.
#   4. Convert the shift back into an actual scale ratio and rotation angle.

# This is an ESTIMATE, not a precise value -- it's used to seed the pyramid
# level selection more sensibly than a manual guess; RANSAC + homography
# still does the final precise geometric solve.
...
# Usage:
#     python fmt_pyramid_lightglue_registration.py
# """
...
# Manual fallback values, only used if FMT's estimate looks unreliable
# (very low phase-correlation response, see FMT_MIN_CONFIDENCE below).
FALLBACK_SOURCE_RESOLUTION_M = 1.0
FALLBACK_REFERENCE_RESOLUTION_M = 1.0

FMT_MIN_CONFIDENCE = 0.05   # phase correlation response below this = don't trust the FMT estimate
...
OUTPUT_DIR = Path("fmt_pyramid_lightglue_output")
...
# ============================================================
# PYRAMID BUILDER  (unchanged from your uploaded code)
# ============================================================
...
print("\n===========================================")
print("  FMT + PYRAMID + SUPERPOINT + LIGHTGLUE REGISTRATION")
print("===========================================")
...
if fmt_confidence < FMT_MIN_CONFIDENCE:
    ...
    # Feed the FMT-estimated ratio directly into the pyramid's resolution
    # parameters -- no manual guessing needed. Reference is the baseline (1.0);
    # source's "effective resolution" is scaled by the estimated ratio.
...
# --- Build pyramids using the FMT-derived resolution ratio ---
...
# --- Load models ---
print("\n[5] Loading SuperPoint + LightGlue models...")
extractor = SuperPoint(max_num_keypoints=MAX_KEYPOINTS).eval().to(DEVICE)
matcher = LightGlue(features="superpoint").eval().to(DEVICE)

# --- Feature detection ---
print("\n[6] Detecting SuperPoint features on matched pyramid levels...")
...
print("\n[7B] Verifying local triangular geometry...")
...
print("\n[9] Registering image (full resolution)...")
...
report = f"""
===========================================
  FMT + PYRAMID + SUPERPOINT + LIGHTGLUE + RANSAC REPORT
===========================================
