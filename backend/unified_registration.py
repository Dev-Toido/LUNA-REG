"""
Unified lunar image registration script for LUNA-REG Workstation.

Combines:
  - SIFT + BFMatcher + Lowe's ratio test + RANSAC
  - Fourier-Mellin Transform (FMT) + Gaussian Pyramid + SuperPoint + LightGlue

Features:
  - Single toggle (FEATURE_METHOD) to switch between SIFT and SuperPoint+LightGlue.
  - Fourier-Mellin log-polar phase correlation for automatic scale & rotation initialization.
  - Resolution pyramid builder to bridge large Ground Sampling Distance (GSD) disparities.
  - Local triangle correspondence filtering to eliminate ambiguous crater/terrain matches.
  - RANSAC homography estimation with spatial grid coverage and sub-pixel reprojection metrics.
  - High-resolution perspective warping, alpha-blended overlay, absolute difference mapping,
    and structured ASCII / CSV diagnostic reporting.

Usage:
    python unified_registration.py
    (GUI dialogs will prompt for reference and moving images)
"""
...
def preprocess_image_sift(image: np.ndarray) -> np.ndarray:
...
def detect_sift(image_gray: np.ndarray):
...
def run_sift_backend(moving_level_img: np.ndarray, reference_level_img: np.ndarray,
...
# Imports are LAZY (only loaded if this method is selected)
# ============================================================
...
def _cv2_to_tensor(image_bgr: np.ndarray, device):
    import torch
...
def _detect_superpoint(extractor, image_bgr: np.ndarray, device):
    import torch
...
def _feats_to_cv2_keypoints(feats, rescale: float = 1.0):
...
def run_superpoint_lightglue_backend(moving_level_img: np.ndarray, reference_level_img: np.ndarray,
                                     source_rescale: float, reference_rescale: float):
...
def get_match_points(matches: List[cv2.DMatch], kp_moving: List[cv2.KeyPoint], kp_reference: List[cv2.KeyPoint]):
...
def triangular_correspondence_filter(matches: List[cv2.DMatch], kp_moving: List[cv2.KeyPoint],
                                     kp_reference: List[cv2.KeyPoint]) -> Tuple[List[cv2.DMatch], np.ndarray]:
    if len(matches) < 6:
...
def estimate_homography(source_points: np.ndarray, reference_points: np.ndarray):
...
def extract_inliers(matches: List[cv2.DMatch], mask: np.ndarray):
    mask = mask.ravel().astype(bool)
    inlier_matches = [m for m, valid in zip(matches, mask) if valid]
    return inlier_matches, mask


def calculate_reprojection_error(source_points: np.ndarray, reference_points: np.ndarray,
...
def calculate_spatial_coverage(points: np.ndarray, image_shape: Tuple[int, ...]):
...
def draw_coverage_map(image: np.ndarray, occupied_cells: set) -> np.ndarray:
    output = image.copy()
...
def run_pipeline(reference_img_path: str = None, moving_img_path: str = None,
                 feature_method: str = FEATURE_METHOD, interactive_display: bool = True):
    print("\n===========================================")
    ...
    if not reference_img_path:
        ...
        reference_img_path = select_image("Select Reference Image")
    print("Reference:", reference_img_path)

    if not moving_img_path:
        print("\n[2] Select MOVING image...")
        moving_img_path = select_image("Select Moving Image")
    print("Moving:", moving_img_path)

    reference_full = load_image(reference_img_path)
    moving_full = load_image(moving_img_path)
...
"""
    ...
    if interactive_display:
        try:
            ...
            print("\nPress any key in an image window to close visualization windows.")
            cv2.waitKey(0)
            cv2.destroyAllWindows()
        except Exception as err:
            print(f"Display window skipped: {err}")
