# """
# SuperPoint + LightGlue based image registration -- drop-in replacement for
# the SIFT feature detection + matching stage in the earlier script. RANSAC,
# triangle verification, spatial coverage, reprojection error, and all output
# generation stay the same.

# Why SuperPoint+LightGlue instead of SIFT (for SIH prototype -- Chandrayaan-2
# TMC-2/OHRC style lunar imagery):
#   - Learned features generalize across illumination differences (sun angle,
#     shadows) far better than SIFT's gradient-based descriptors.
#   - LightGlue uses global attention (it looks at the whole image's keypoint
#     layout, not just local patches), so it produces fewer wrong matches on
#     repetitive terrain like craters.
#   - Apache-2.0 licensed (unlike SuperGlue, which is non-commercial-research
#     only) -- safe to use in a hackathon/product prototype.
...
# Usage:
#     python lightglue_registration.py
#     (file picker dialogs will prompt you for the reference and moving image)
# """
...
MAX_KEYPOINTS = 4096            # SuperPoint's equivalent of SIFT's nfeatures
MIN_MATCH_CONFIDENCE = 0.0      # optional extra filter on LightGlue's own match scores (0-1)
...
# Local triangular correspondence verification (unchanged from SIFT version)
...
# ============================================================
# SUPERPOINT FEATURE DETECTION  (replaces detect_sift)
# ============================================================
...
def feats_to_cv2_keypoints(feats):
    ...
    so the rest of the pipeline (drawMatches, triangle verification, etc.)
    can stay unchanged."""
...
# ============================================================
# LIGHTGLUE MATCHING  (replaces match_sift / match_sift_bidirectional)
# ============================================================

def match_lightglue(matcher, feats_moving, feats_reference):
    """
    LightGlue performs a learned, globally-optimal assignment (its
    attention layers already look at both images jointly), so unlike
    BFMatcher+ratio-test it does NOT need a separate bidirectional /
    mutual-consistency pass -- that consistency is effectively built in.
    """
    with torch.no_grad():
    ...
    for (i, j), score in zip(match_indices, match_scores):
        if score < MIN_MATCH_CONFIDENCE:
            continue
        # distance = 1 - confidence, so downstream code (which expects
        # "lower distance = better match", same convention as SIFT's L2
        # distance) keeps working unchanged, e.g. in triangle verification's
        # sort-by-distance step.
...
# ============================================================
# CONVERT MATCHES TO COORDINATES  (unchanged)
# ============================================================
...
# ============================================================
# LOCAL TRIANGULAR CORRESPONDENCE VERIFICATION  (unchanged)
# ============================================================
...
# --- Load models once ---
print("\n[3] Loading SuperPoint + LightGlue models...")
extractor = SuperPoint(max_num_keypoints=MAX_KEYPOINTS).eval().to(DEVICE)
matcher = LightGlue(features="superpoint").eval().to(DEVICE)

# --- Feature detection ---
print("\n[4] Detecting SuperPoint features...")
feats_reference = detect_superpoint(extractor, reference)
feats_moving = detect_superpoint(extractor, moving)

kp_reference = feats_to_cv2_keypoints(feats_reference)
kp_moving = feats_to_cv2_keypoints(feats_moving)
print("Reference keypoints:", len(kp_reference))
print("Moving keypoints:", len(kp_moving))

reference_keypoints_image = cv2.drawKeypoints(reference, kp_reference, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
moving_keypoints_image = cv2.drawKeypoints(moving, kp_moving, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
...
correspondence_image = cv2.drawMatches(moving, kp_moving, reference, kp_reference, good_matches, None,
...
triangle_correspondence_image = cv2.drawMatches(moving, kp_moving, reference, kp_reference, good_matches, None,
                                                 flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
...
moving_coverage, moving_cells = calculate_spatial_coverage(inlier_source_points, moving.shape)
reference_coverage, reference_cells = calculate_spatial_coverage(inlier_reference_points, reference.shape)
print(f"Moving inlier coverage: {moving_coverage:.2f}%")
print(f"Reference inlier coverage: {reference_coverage:.2f}%")

ransac_inlier_image = cv2.drawMatches(moving, kp_moving, reference, kp_reference, inlier_matches, None,
                                       flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
cv2.imwrite(str(OUTPUT_DIR / "06_ransac_inliers.png"), ransac_inlier_image)

moving_coverage_image = draw_coverage_map(moving, moving_cells)
reference_coverage_image = draw_coverage_map(reference, reference_cells)
...
report = f"""
===========================================
     SUPERPOINT + LIGHTGLUE + RANSAC REPORT
===========================================
...
LightGlue Confident Matches : {len(inlier_matches) + (len(good_matches) - inlier_count)}
