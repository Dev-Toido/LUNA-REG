# """
# Pyramid-aware SuperPoint + LightGlue registration.

# Merges two pieces:
#   1. PyramidBuilder -- builds multi-scale Gaussian pyramids for both images
#      and finds which pyramid levels have approximately EQUAL effective
#      resolution (e.g. bridging OHRC 0.25m vs TMC-2 5m).
#   2. SuperPoint + LightGlue -- runs feature detection + matching on the
#      LEVEL-MATCHED images (not full resolution), so the scale gap between
#      sensors doesn't overwhelm the matcher. Matched keypoint coordinates
#      are then rescaled back to full resolution before RANSAC/warping, so
#      the final registered/warped output is still full-resolution.

# If your two images already have the same resolution, just leave
# SOURCE_RESOLUTION_M = REFERENCE_RESOLUTION_M (default) -- the pyramid will
# collapse to a single level (full resolution), i.e. this behaves exactly
# like the plain lightglue_registration.py script.
...
# Usage:
#     python pyramid_lightglue_registration.py
#     (file picker dialogs prompt for the reference and moving image)

# IMPORTANT: set SOURCE_RESOLUTION_M / REFERENCE_RESOLUTION_M below to your
# actual sensor resolutions (meters/pixel) before running, if they differ.
# """
...
# --- Resolution info for pyramid scale matching ---
# Set these to your MOVING and REFERENCE image's real-world resolution
# (meters/pixel, or any consistent unit). If both images are the same
# resolution, leave both at 1.0 -- pyramid will just use level 0 (full res).
SOURCE_RESOLUTION_M = 0.25      # e.g. OHRC ~0.25 m/pixel
REFERENCE_RESOLUTION_M = 5.0    # e.g. TMC-2 ~5 m/pixel

PYRAMID_SCALE_FACTOR = 0.5
PYRAMID_MAX_LEVELS_CAP = 6      # safety cap, don't build absurdly many levels
...
OUTPUT_DIR = Path("pyramid_lightglue_output")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)


# ============================================================
# PYRAMID BUILDER  (from your uploaded code, unchanged)
# ============================================================
...
def feats_to_cv2_keypoints(feats, rescale=1.0):
    """rescale: multiply keypoint coords by this to convert from the
    pyramid-level's pixel space back to FULL-RESOLUTION pixel space
    (rescale = 1 / level_scale_factor)."""
    kps = feats["keypoints"][0].cpu().numpy()
    scores = feats["keypoint_scores"][0].cpu().numpy()
    cv_keypoints = [
        cv2.KeyPoint(float(x * rescale), float(y * rescale), size=6, response=float(s))
        for (x, y), s in zip(kps, scores)
    ]
    return cv_keypoints


def match_lightglue(matcher, feats_moving, feats_reference):
    with torch.no_grad():
        result = matcher({"image0": feats_moving, "image1": feats_reference})
    feats_moving_r, feats_reference_r, result = [rbd(x) for x in [feats_moving, feats_reference, result]]
...
# ============================================================
# TRIANGLE VERIFICATION  (unchanged)
# ============================================================
...
def triangular_correspondence_filter(matches, kp_moving, kp_reference):
    ...
    if len(filtered_matches) < MIN_MATCHES:
        print("Triangle verification retained too few matches; falling back to LightGlue matches.")
...
# ============================================================
# RANSAC / INLIERS / REPROJECTION ERROR / COVERAGE  (unchanged)
# ============================================================
...
print("\n===========================================")
print("  PYRAMID-AWARE SUPERPOINT + LIGHTGLUE REGISTRATION")
print("===========================================")
...
# --- Build pyramids ---
print("\n[3] Building pyramids and finding matching resolution levels...")
builder = PyramidBuilder()

n_levels = builder.compute_levels_for_scale_ratio(SOURCE_RESOLUTION_M, REFERENCE_RESOLUTION_M, PYRAMID_SCALE_FACTOR)
...
matching_pairs = builder.find_matching_levels(
    source_pyramid, reference_pyramid, SOURCE_RESOLUTION_M, REFERENCE_RESOLUTION_M
)

if matching_pairs:
    source_level_idx, reference_level_idx = matching_pairs[0]  # finest matching pair
...
# Rescale factors to convert level-space keypoints back to FULL resolution
...
# --- Load models ---
print("\n[4] Loading SuperPoint + LightGlue models...")
extractor = SuperPoint(max_num_keypoints=MAX_KEYPOINTS).eval().to(DEVICE)
matcher = LightGlue(features="superpoint").eval().to(DEVICE)

# --- Feature detection on the MATCHED PYRAMID LEVEL images ---
print("\n[5] Detecting SuperPoint features on matched pyramid levels...")
feats_reference = detect_superpoint(extractor, reference_level_img)
feats_moving = detect_superpoint(extractor, moving_level_img)

# Keypoints rescaled back to FULL-RESOLUTION coordinate space immediately,
# so everything downstream (drawing on full images, RANSAC, warping) works
# in a single consistent coordinate system.
kp_reference = feats_to_cv2_keypoints(feats_reference, rescale=reference_rescale)
kp_moving = feats_to_cv2_keypoints(feats_moving, rescale=source_rescale)
...
# --- LightGlue matching ---
print("\n[6] Matching with LightGlue...")
total_candidates, good_matches = match_lightglue(matcher, feats_moving, feats_reference)
print("Total candidate keypoint pairs considered:", total_candidates)
print("LightGlue confident matches:", len(good_matches))

if len(good_matches) < MIN_MATCHES:
    raise RuntimeError("Registration rejected: not enough reliable LightGlue matches.")

correspondence_image = cv2.drawMatches(moving_full, kp_moving, reference_full, kp_reference, good_matches, None,
                                        flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
cv2.imwrite(str(OUTPUT_DIR / "05_lightglue_correspondences.png"), correspondence_image)

# --- Triangle verification (in full-resolution coordinate space) ---
print("\n[6B] Verifying local triangular geometry...")
...
# --- RANSAC (full-resolution coordinates) ---
...
# --- Warp full-resolution moving image onto full-resolution reference frame ---
print("\n[8] Registering image (full resolution)...")
...
report = f"""
===========================================
  PYRAMID + SUPERPOINT + LIGHTGLUE + RANSAC REPORT
===========================================
...
Source resolution (m/px)      : {SOURCE_RESOLUTION_M}
Reference resolution (m/px)   : {REFERENCE_RESOLUTION_M}
...
-------------------------------------------
FEATURE DETECTION (SuperPoint, on matched levels)
-------------------------------------------
...
Candidate Keypoint Pairs : {total_candidates}
...
cv2.imshow("LightGlue Correspondences", resize_for_display(correspondence_image))
