...
# ============================================================
# EXPERIMENT / QUALITY SETTINGS
# ============================================================

# Test A = same effective scale
# Test B = different resolution / scale
# Test C = different scale + illumination difference

TEST_MODE = "A"       # Change to "A", "B", or "C"

# Blur detection
BLUR_THRESHOLD = 50.0

# Registration acceptance
MIN_INLIER_RATIO = 35.0   # 35%

# Minimum number of good matches
MIN_MATCHES = 10

# Minimum absolute RANSAC inliers
MIN_INLIERS = 8
...
def check_image_blur(image, image_name):
    """
    Check whether an image is sufficiently sharp.
    """

    score = calculate_blur_score(image)

    print(f"{image_name} blur/sharpness score: {score:.2f}")
    print(f"Required minimum score: {BLUR_THRESHOLD:.2f}")

    if score < BLUR_THRESHOLD:
        print()
        print("===========================================")
        print(f"❌ {image_name} IMAGE IS TOO BLURRY")
        print("===========================================")
    ...
    print(f"✅ {image_name} image is sufficiently sharp.")
    return True
...
if TEST_MODE == "A":
    print("TEST A: Same effective scale")

elif TEST_MODE == "B":
    print("TEST B: Different resolution / scale")

elif TEST_MODE == "C":
    print("TEST C: Different scale + illumination difference")

else:
    raise ValueError("TEST_MODE must be 'A', 'B', or 'C'")
...
print("\n[2B] Checking image quality...")

reference_clear = check_image_blur(
    reference_full,
    "REFERENCE"
)

moving_clear = check_image_blur(
    moving_full,
    "MOVING"
)

if not reference_clear or not moving_clear:
    print("❌ At least one image failed the blur check.")
    print("❌ Registration will NOT continue.")
    raise SystemExit

print("✅ Both images passed the blur check.")
...
print("\n===========================================")
print("         FINAL MATCHING DECISION")
print("===========================================")

print(f"RANSAC Inliers : {inlier_count}")
print(f"Good Matches   : {len(good_matches)}")
print(f"Inlier Ratio   : {inlier_ratio:.2f}%")
print(f"Required Ratio : {MIN_INLIER_RATIO:.2f}%")

if inlier_ratio < MIN_INLIER_RATIO:

    print()
    print("❌ IMAGES ARE NOT MATCHING")
    print(f"❌ Inlier ratio {inlier_ratio:.2f}% is below "
          f"the required {MIN_INLIER_RATIO:.2f}%")
    ...
    print(f"✅ Inlier ratio {inlier_ratio:.2f}% >= "
          f"{MIN_INLIER_RATIO:.2f}%")
    print("✅ Registration will continue.")
    print()
